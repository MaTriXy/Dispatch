...
internal class WithViewLifecycleScopeEveryTest {
  ...
  fun `when the Fragment lifecycle is only created, lambda should be invoked`() {
    ...
    val fragment =
      TestFragment(createdFragmentLifecycleOwner) { withViewLifecycleScope(policy = EVERY) { invocations++ } }
  ...
  fun `when the livedata goes from null to non-null, lambda should be invoked`() {
    ...
    val fragment =
      TestFragment(fragmentLifecycleOwner) { withViewLifecycleScope(policy = EVERY) { invocations++ } }
  ...
  fun `when the view lifecycle is destroyed, lambda should be cancelled`() {
    ...
    val fragment = TestFragment(fragmentLifecycleOwner) {
      withViewLifecycleScope(policy = EVERY) {
        job = launch {
